...
const NotPicked = () => {
  ...
  useEffect(() => {
    const storedOrders = JSON.parse(localStorage.getItem('notPickedOrders')) || [];
  ...
  const handleDelete = () => {
    ...
    localStorage.setItem('notPickedOrders', JSON.stringify(updatedOrders));
    setSelectedOrders([]);
  };

  return (
    <div className='order_list-panel'>
      <h3 className='list_title'>Не забраті</h3>
