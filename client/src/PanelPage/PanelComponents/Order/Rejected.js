...
const Rejected = () => {
  ...
  useEffect(() => {
    const storedOrders = JSON.parse(localStorage.getItem('rejectedOrders')) || [];
  ...
  const handleDelete = () => {
    ...
    localStorage.setItem('rejectedOrders', JSON.stringify(updatedOrders));
    setSelectedOrders([]);
  };

  return (
    <div className='order_list-panel'>
      <h3 className='list_title'>Відхилені</h3>
