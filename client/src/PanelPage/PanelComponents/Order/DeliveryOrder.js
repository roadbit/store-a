...
const DeliveryOrder = () => {
  ...
  const moveOrdersToReceived = () => {
    const updatedOrders = orders.filter(order => !selectedOrders.includes(order.orderNumber));
    const ordersToMove = orders.filter(order => selectedOrders.includes(order.orderNumber));
    
    localStorage.setItem('deliveryOrders', JSON.stringify(updatedOrders));
    localStorage.setItem('receivedOrders', JSON.stringify([...JSON.parse(localStorage.getItem('receivedOrders')) || [], ...ordersToMove]));

    setOrders(updatedOrders);
    setSelectedOrders([]);
  };

  const moveOrdersToNotPicked = () => {
    const updatedOrders = orders.filter(order => !selectedOrders.includes(order.orderNumber));
    const ordersToMove = orders.filter(order => selectedOrders.includes(order.orderNumber));

    localStorage.setItem('deliveryOrders', JSON.stringify(updatedOrders));
    localStorage.setItem('notPickedOrders', JSON.stringify([...JSON.parse(localStorage.getItem('notPickedOrders')) || [], ...ordersToMove]));
  ...
  return (
    <div className='order_list-panel'>
      <h3 className='list_title'>Відправлені</h3>
      <div className="button_list-line">
        <div>
          <button onClick={moveOrdersToReceived} disabled={selectedOrders.length === 0}>
            Замовлення отримане
          </button>
          <button onClick={moveOrdersToNotPicked} disabled={selectedOrders.length === 0}>
            Не забрате
          </button>
        </div>
      </div>
