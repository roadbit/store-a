...
const CreateOrder = () => {
  ...
  useEffect(() => {
    const storedOrders = JSON.parse(localStorage.getItem('createOrders')) || [];
  ...
  const handleMoveToDelivery = () => {
    ...
    localStorage.setItem('createOrders', JSON.stringify(updatedOrders));
    ...
    localStorage.setItem('deliveryOrders', JSON.stringify([...existingOrders, ...movedOrders]));
  ...
  return (
    <div className='order_list-panel'>
      <h3 className='list_title'>Підтверджені</h3>
      <div className="button_list-line">
        <button onClick={handleMoveToDelivery} disabled={selectedOrders.length === 0}>
          ...
          orders.map((order, index) => (
            <li className='order_list-li' key={index}>
              ...
              <div className="descr_order-list">
                ...
                <div className="descr_person-list"></div>
                <p>Ім'я: {order.name}</p>
